        ...
        <div class="h-5/6 flex flex-col justify-between">
            <div class="pt-10 px-3 text-white h-5/6 overflow-y-auto">
                <div v-for="msg in messages" :key="msg.timestamp" class="mb-2 ">
                    ...
                    <p :class="{'text-right': msg.senderId === props.sid, 'text-left': msg.senderId === props.rid}">
                        <span class="p-2 rounded bg-blue-700">{{ msg.content }}</span>
                    </p>
                </div>
            </div>
            <div class="flex flex-row justify-end h-8 pr-4">
                <input type="text" v-model="message" class="border bg-transparent w-3/5">
...
<script setup>
    ...
    ) || parentData.parents.find(
        (parent) => parent.id === props.rid
    );
